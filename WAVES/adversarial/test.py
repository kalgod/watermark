...
def pgd_attack(model, criterion, images, labels, eps=8/255, alpha=None, iters=10, norm=None, noise=None):
    ...
    for k in range(iters):
        ...
        print(k,"loss: {:.4f}".format(loss.item()),outputs,torch.round(outputs))
...
def load_dataset(transform, path, y=1):
    dataset = CustomImageFolder(path, transform=transform, y=y,data_cnt=100)
    N = len(dataset)
...
def apply_adv(args, mode='wm', eps=8):
    ...
    transform = transforms.Compose([transforms.CenterCrop((args.image_size, args.image_size)), transforms.ToTensor()])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--defense", default="advmark", type=str)
    args = parser.parse_args()

    if (args.defense in ["DwtDctSvd","stega"]): args.image_size=256
