...
def load_dataset(args):
    transform = transforms.Compose([transforms.CenterCrop((args.image_size, args.image_size)), transforms.ToTensor(), AddGaussianNoise(0., 0.4)])
...
def main():
    ...
    if (args.defense in ["DwtDctSvd","stega"]): 
        args.image_size=256
        args.batch_size=250
    ...
    cp = torch.load("./models/"+args.defense+".pth")
    if (args.load==1): model.load_state_dict(cp['model'])
    model.eval()
